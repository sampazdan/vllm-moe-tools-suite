...
def _run_zipper(payload: dict[str, object], module: object) -> dict[str, object]:
    zipper_type = module.Zipper
    value = zipper_type.from_tree(payload["tree"])
    for step in payload.get("steps", []):
        value = getattr(value, step["method"])(*step.get("args", []))
    if value is None:
        return {"kind": "none", "status": "returned"}
    if isinstance(value, zipper_type):
        return {"kind": "zipper", "status": "returned"}
    return {"kind": "value", "status": "returned", "value": value}
...
def main() -> None:
    ...
    try:
        with contextlib.redirect_stdout(sink), contextlib.redirect_stderr(sink):
            ...
            elif payload["mode"] == "zipper":
                result = _run_zipper(payload, module)
            else:
