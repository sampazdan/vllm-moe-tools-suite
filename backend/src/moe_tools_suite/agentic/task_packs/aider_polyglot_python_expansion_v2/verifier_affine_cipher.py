...
def encode(text: str, a: int, b: int) -> str:
    return call("affine_cipher.py", "encode", [text, a, b])


def decode(text: str, a: int, b: int) -> str:
    return call("affine_cipher.py", "decode", [text, a, b])
...
class AffineCipherTest(unittest.TestCase):
    ...
    def test_encode_with_a_not_coprime_to_m(self):
        with self.assertRaisesRegex(ValueError, "a and m must be coprime\\."):
