...
if [[ "${template_mode}" != "mock" && "${template_mode}" != "vllm" && "${template_mode}" != "agentic" && "${template_mode}" != "v2" ]]; then
    echo "MOE_TOOLS_TEMPLATE_MODE must be mock, vllm, agentic, or v2." >&2
    exit 2
fi
...
elif [[ "${template_mode}" == "v2" ]]; then
    echo "MOE_TOOLS_IMAGE must use an immutable sha256 digest for V2." >&2
    exit 2
...
if [[ "${template_mode}" == "v2" ]]; then
    template_source="${repository_root}/deploy/runpod-v2-template.example.json"
    runtime_mode="vllm"
elif [[ "${template_mode}" == "agentic" ]]; then
